...
void AFE_Reset(void) {
    ...
    */
	...
	if(!System_ERROR_UserCallback(ERROR_STATUS_AFE1)) {
	    if(!MTPWrite(0xEA, 1, WrBuf)) {              //0xEA, 0xC0?A CRC
            MTPWrite(0xEA, 1, WrBuf);
        }
    }
}
...
UINT8 AFE_IsReady(void) {
	...
	while(1) {
		...
		TempVar = 0;
		if(MTPRead(MTP_BFLAG2, 1, &TempVar)) {		//读取BLFG2，查看VADC是否转换完成
...
UINT8 SH367309_SC_DelayT_Set(void) {
	...
	if(u8temp_need > 15)u8temp_need = 15;
		...
		*/
        for(i = 0; i < 25; i++) {                                //最后一个TR不做对比
            if(bufferbak[i] != mtpbufferbak[i]) {
                ...
                if(AFE_CalcuVbat() < 18000) {		//如果AFE供电低于18V，烧写时VPRO电压可能不足8V，不能烧写
                    System_ERROR_UserCallback(ERROR_AFE1);
                    break;
                }
                ...
                MCUO_AFE_VPRO = 1;
				Delay1ms(20);
                if(!MTPWriteROM(0x00, 25, mtpbufferbak)) {     //重写EEPROM的寄存器，两次
                    //System_ERROR_UserCallback(ERROR_AFE1);
                }
                MCUO_AFE_VPRO = 0;
				Delay1ms(1);
...
void SH367309_Driver_Supplement(void) {
	...
	switch(su8_MOS_DSG_Status) {
		case 0:
			//刚开机也是关闭，所以问题不大。
			//运行期间检查到关闭，也要执行一遍循环
			if(!gu8_DriverStartUpFlag) {		//把电流校准放在前面
				return;
			}
		...
		case 1:
			...
			else if(SystemStatus.bits.b1Status_BnCloseIO) {			//如果有别的手动关闭的信号，必须添加到这里
				DSG_Close_Flag = 1;
			}
			else {
		...
		case 3:
			...
			if(++su32_Delay_Cnt >= 10) {	//延时100ms关闭预充
				su32_Delay_Cnt = 0;
		...
		case 4:
			if(SystemStatus.bits.b1Status_MOS_DSG == OPEN) {	//监控是否真的打开了，打开才回到原来的地方监控。
				su8_MOS_DSG_Status = 0;
				su8_OpenT_Cnt = 0;
			}
			
			if(++su8_OpenT_Cnt >= 200) {						//如果计时2s内没打开，则报错
				su8_OpenT_Cnt = 0;
...
void App_SH367309_Monitor(void) {
	...
	if(0 == g_st_SysTimeFlag.bits.b1Sys100msFlag) {		//这个时基不能随便调，影响MOS动作，初始化电流校准
		return;
	}
